# M003 -- Data Format & Interop Execution Plan
...
M003 runs after M002 (LXC Release & Stability) and M004 (Type Safety, Decomposition & Stability), and before M014 (UX shell) and M016 (keyboard pass). M015 (storage) runs in parallel and does not gate M003. The working sequence is M002 -> M004 -> M003 -> M014 -> M016.

**Cross-milestone gates in**

- M002 shipped the narrow #2152 patch on the existing slot machinery (restore half-U stacking). #2158 deletes that machinery; the patch must already be on main before the epic implementation starts.
- M004 #2146 landed an interim no-op guard for arrow-key nudge of contained devices in src/lib/utils/device-movement.ts (findNextValidPosition). #2158 owns the end state and replaces that guard.
- M004 #1419/#1420 E2E selector infrastructure lands before M003; the E2E portions of #1114 lean on it.
- #617 (images in YAML) moved to M015 for data-safety reasons; nothing in M003 waits on it. #618 is closed.

**Cross-milestone gates out**

- M014 #2075 (device verb bar half-width slot control) is blocked on #2158's slot-model decision; all M014 placement and drag work sequences after this milestone.
- M014 #2095 (templates) becomes a CI-validated consumer of the schema artifact published by #571.
- M008 #820 (share-link shortener) consumes the policy from #2186. M008 #2181 (git-sync regression coverage, split from #1114) waits on #627 in M008.
- M010 takes GeoJSON export, trimmed out of #1209.
- #2164 (drive-bay spike, unmilestoned) is sequenced after #2158's design freeze.
- #1208 (Initiative: Ecosystem Interoperability) moved to Backlog; #1209 proceeds in M003 without it.
...
- Blockers: none.
- Why this position: share-URL payloads are a schema read surface with no compatibility policy, and the answer feeds both #1113's compatibility matrix and M008 #820's design. It must conclude before #2158's share-link format revision.
...
- Scope: generate JSON Schema from the Zod schemas in src/lib/schemas/index.ts (zod-to-json-schema or similar), publish versioned files at a stable raw GitHub URL, and integrate generation into the build with a CI sync check. Audit AC: schema version naming follows #1113's policy and is decoupled from app CalVer; refresh the stale pre-CalVer examples in the issue body (schema/v0.6.json; no schema/ directory exists in the repo, the issue creates it). Documentation minimum: canonical schema URL, one YAML sample showing $schema usage, VS Code + yaml-language-server setup notes, and known validation limitations (refine/cross-field rules are not expressible). Templates (#2095, M014) become CI-validated consumers of this artifact.
...
- Why this position: audit re-scope keeps only the YAML round-trip and legacy ZIP load half here (the YAML default shipped in PR #1754); the git-sync half is #2181 in M008. This is the remaining precondition gating #620.
- Scope: automated coverage for YAML save plus reload round-trip of representative layouts, and for loading legacy ZIP files with compatibility behaviour preserved. Include pre-bump fixtures so the #2158 import adapter path stays covered. E2E portions lean on the M004 #1419/#1420 selector infrastructure already on main. Document the coverage in docs/guides/TESTING.md. No live external dependencies.
...
- Scope: audit-corrected AC: delete only the single-layout ZIP save path; retain createFolderArchive/getJSZip because export-all (#2045, M014) builds on downloadArchive and export.ts dynamically imports ZIP creation for multi-rack PNG export, a live write consumer. Verify JSZip is lazy-loaded only when a ZIP file is detected and measure bundle impact (JSZip must not be in the initial bundle). Legacy ZIP read is kept indefinitely.
...
- Why this position: audit re-scope: the epic's ACs no longer include the git-sync MVP (all git-sync issues #627/#628/#629 live in M008, plus #2181), and #617 (images in YAML) moved to M015. The epic closes with M003.
- Scope: verify the audit close conditions: schema versioning policy (#1113), schema bump (#2158), published schema (#571), the YAML half of #1114, and #620 are done. Confirm remaining open questions are resolved or moved into tracked follow-up issues (M008: #627/#628/#629/#2181/#820; M015: #617; Backlog: #794/#795 device type ecosystem research per the epic checklist). Close the epic with a comment linking the delivered artifacts.
...
- [ ] All M003 issues closed: #570, #571, #620, #1113, #1114, #1209, #2131, #2158, #2165, #2186 (gh issue list -R RackulaLives/Rackula --milestone "M003 -- Data Format & Interop" --state open returns nothing).
...
- [ ] Gates out honoured: #2075 (M014) unblocked note posted, #2095 schema-consumer follow-up visible, #820/#2181 (M008) reference the M003 outcomes, GeoJSON tracked in M010.
