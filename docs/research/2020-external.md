...
Browser-based Svelte 5 rack-layout app (canvas, direct-manipulation, client-side + server build). Research date: 2026-06-13. All claims web-sourced; see Key Sources.
...
- **VS Code**: Command Palette (`Ctrl+Shift+P`) runs commands; **Quick Open (`Ctrl+P`)** is a separate, file/entity-navigation surface. They are deliberately different shortcuts. You _can_ unify - typing `>` in Quick Open jumps to command mode - but the default surfaces are split.
